...
- What did you learn?
    ...
    - WebSocket: We had to learn how to use WebSocket to create a live chat. WebSocket aloows for live, 2-way connections between a user's browser and a server. This connection has a variety of uses, though we used it to make a live chat usable by anyone who logs into our website. While WebSockets is useful for live connections, it's difficult to coordinate multiple connections to one server all at once. Perfectly doable, though not within the timeframe of the project, so for the sake of efficiency, the library socket.io was utilized to streamline the process of making the chat accessible to all players and update every player connected.
